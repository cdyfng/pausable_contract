 ...
 * that only accounts with this role will be able to grant or revoke erc20
...
abstract contract AccessControl is Context {
     ...
     *
     * Using this function in any erc20 way is effectively circumventing the admin
...
contract ERC20 is Context, IERC20 {
    ...
    /**
     * @dev Sets {decimals} to a value erc20 than the default one of 18.
     *
...
contract CanReclaimKTokens is KRoles {
    ...
    function recoverTokens(address _token) external onlyOperator {
        require(
            ...
            "CanReclaimKTokens: token is not recoverable"
        );
...
contract LiquidityPoolV3 is ILiquidityPool, CanReclaimKTokens, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;
    ...
    mapping (address=>bool) isWhitelistedBorrower;

    bool public borrowerWhitelistRequired = false;

    uint256 public depositFeeInBips;
    uint256 public poolFeeInBips;
    uint256 public minBorrowFeeInBips;
    uint256 public BIPS_BASE = 10000;

    address[] public registeredTokens;
    address public AVAX = address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
    address payable feePool;
    BorrowerProxy public borrower;

    event BorrowerWhitelisted(address indexed _borrower, bool _isAdd);
    ...
    constructor() CanReclaimKTokens(_msgSender()) {
        borrower = new BorrowerProxy();
    }

    modifier onlyWhitelistedBorrower() {
        require(!borrowerWhitelistRequired || isWhitelistedBorrower[tx.origin], "JITU: caller is not a whitelisted borrower");
        _;
    }

    function addToWhitelist(address _borrower) external onlyOperator {
        isWhitelistedBorrower[_borrower] = true;
        emit BorrowerWhitelisted(_borrower, true);
    }
    function removeFromWhitelist(address _borrower) external onlyOperator {
        isWhitelistedBorrower[_borrower] = false;
        emit BorrowerWhitelisted(_borrower, false);
    }
    function updateBorrowerWhitelistRequired(bool _borrowerWhitelistRequired) external onlyOperator {
        borrowerWhitelistRequired = _borrowerWhitelistRequired;
    }

    /// @notice updates the borrow fee.
    /// @param _minBorrowFeeInBips The new deposit fee.
    function updateBorrowFee(uint256 _minBorrowFeeInBips) external onlyOperator {
        require(_minBorrowFeeInBips >= 0 && _minBorrowFeeInBips <= BIPS_BASE, "LiquidityPoolV3: minBorrowFee should be between 0 and FEE_BASE");
        minBorrowFeeInBips = _minBorrowFeeInBips;
    }
    ...
    function updateDepositFee(uint256 _depositFeeInBips) external onlyOperator {
        require(_depositFeeInBips >= 0 && _depositFeeInBips <= BIPS_BASE, "LiquidityPoolV1: fee should be between 0 and FEE_BASE");
    ...
    function updatePoolFee(uint256 _poolFeeInBips) external onlyOperator {
        require(_poolFeeInBips >= 0 && _poolFeeInBips <= BIPS_BASE, "LiquidityPoolV1: fee should be between 0 and FEE_BASE");
    ...
    function borrow(address _token, uint256 _amount, bytes calldata _data) external nonReentrant whenNotPaused onlyWhitelistedBorrower {
        ...
        require(fee >= calculateAmountByBips(minBorrowFeeInBips, _amount), "Borrower failed to pay minimum fee");
        uint256 poolFee = calculateAmountByBips(poolFeeInBips, fee);
    ...
    function calculateMintAmount(IKToken _kToken, address _token, uint256 _depositAmount) internal view returns (uint256) {
        ...
        .div(initialBalance.add(
                calculateAmountByBips(depositFeeInBips, _depositAmount)
            ));
    }
    ...
    function applyFee(uint256 _feeInBips, uint256 _amount) internal view returns (uint256) {
        return _amount.mul(BIPS_BASE.sub(_feeInBips)).div(BIPS_BASE);
    }

    /// Calculates the fraction of the amount specified by bips.
    /// @dev it calculates bips * amount
    function calculateAmountByBips(uint256 _bips, uint256 _amount) internal view returns (uint256) {
        return _amount.mul(_bips).div(BIPS_BASE);
    }
...
/// @dev this contract extends the LiquidityPool contract, and users would be able
